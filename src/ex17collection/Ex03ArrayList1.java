...
public class Ex03ArrayList1 {
	...
	public static void main(String[] args) {
		/*
		 ...
		 ArrayList와 나머지 컬렉션들은 데이터를 저장하는 내부적인 자료구조인 다를뿐 사용법은 100%동일하다.
		 */
		...
		/*
		 객체저장 : add()
		 	...
		 	add(인덱스, 인스턴스) :  데이터 저장시 인덱스를 직접부여한다.
		 						단, 인덱스를 건너띄면 에러가 발생하므로 주의해야한다.
		 						만약 중복된 인덱스가 있다면 삽입(끼워넣기)이된다. 
		 */
		list.add("소녀시대");
		list.add("빅뱅");
		list.add("트와이스");
...
//		list.add(5, "소방차");//에러발생 건너띄면 안된다. 
		...
		/*
		 중복저장 : List는 배열의 특성을 가지므로 데이터를 중복저장 할 수 있다. 동일한 데이러타도 index로 구분할 수 있기 때문이다.
		...
		/*
		 ...
		 : 해당 인덱스에 인스턴스를 삽입한다. 기존인스턴스는 뒤로 1칸씩 밀려난다. 
		   즉, 자동으로 인덱싱된다.  
		 */
		...
		/*
		 포함여부확인
		 	contains(인스턴스) : 특정 인스턴스가 저장되어 있는지 확인할 때 사용한다. 존재한다면 true를 반환한다.  
		 */
		...
		/*
		 ...
		 방법1 : 인덱스로 삭제
		 		remove(인덱스) : 인덱스를 통해 삭제를 진행한 후 
		 					   ...
		 					   삭제 후에는 인덱스가 자동으로 부여된다. 삭제 성공시 삭데된 인스턴스를
		...
		System.out.println("\n====================================================\n");
		/*
		 위치를 모르는 경우네는 indexOf()를 통해 해당 인스턴스의 인덱스를 찾은 후 삭제한다.
		 */
		...
		System.out.println("\n====================================================\n");
		/*
		 방법2 : 인스턴스의 참조값을 통해 삭제한다. 이 경우에는 삭제에 성공하면 true가 반환한다. 
