...
public class Ex06HashMapMain {
	...
	public static void main(String[] args) {
		...
		/*
		 인스턴스 저장 : 최초 저장이라면 성공시 null을 반환한다. 만약 기존에 이미 저장된 동일한 key가 존재하면
		 			 저장된 인스턴스가 반환된다. 
		 */
...
//		map.put("age", number);
		...
		/*
		 출력하기
		 1. Key를 알고 있을때 : get(key)으로 출력한다.
		 */
		System.out.println("키값을 알때:" + map.get("name"));
		
		/*
		 2. Key를 모를 때(혹은 너무 많아서 하나씩 명시하기 힘들때)
		 :keySet()을 통해 전체 Key를 얻어온다. 
		  이를 통해 for문으로 순차적인 접근을 하면된다.
		 */
		Set<String> keys = map.keySet();
		System.out.println("[확장for문 적용]");
		for(String key : keys) {
			//get(key값)을 통해 value을 출력한다. 
		...
		/*
		 존재유무 확인
		 인스턴스의 존재유무 확인
		 :Map컬렉션은 Key와 Value가 있으므로 메서드도 아래와 같이 2개로 나눠져있다. 
		 */
		System.out.println(map.containsKey("name") ? "name키있다." : "name키없다.");
		System.out.println(map.containsKey("account") ? "account키있다." : "account키없다.");
		...
		/*
		 전체삭제 : removeAll은 없고,  clear()만 사용할 수 있다. 
		 */
		map.clear();
