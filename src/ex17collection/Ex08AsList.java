...
public class Ex08AsList {
	...
	public static void listPrint(String title, List<String> list) {
		...
		/*
		 확장 for문으로 전체를 출력한다. 출력 할 인스턴스가 String이므로
		 별도의 오버라이딩 없이도 내용을 출력할 수 있다. 만약 우리가 직접 정의한
		 클래스라면 toString()을 오버라이딩 해야한다. 
		 */
		for(Object ob : list) {
			System.out.print(ob + " ");
		}
	...
	public static void main(String[] args) {
		...
		/*String 인스턴스 배열을 생성과 동시에 초기화한다. 
		  이 경우 초기데이터를 통해 크기를 유추할 수 있으므로 크기는 생략해야한다.*/
		String[] strArr = new String[] {"마린", "파이어뱃", "메딕", "마린"};
		
		/*
		 Arrays.asList() : 일반적인 배열을 List<E> 컬렉션으로
		 				   변경한 후 반환한다. 단, 값을 참조만 할 수 있고 입력이나 삭제는 할 수 없다. 
		 */
...
//			실행문장 => next()가 증감식을 대체하는 형식으로 작성한다.
		...
		/*
		 ...
		 단 출력순서는 변경 될 수 있다. (순서를 보장하지 않기때문)
		 */
