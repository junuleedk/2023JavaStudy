...
public class Ex03ArrayList2 {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 인스턴스 생성시 Student 타입매개변수를 통해 생성한다. 즉, Student인스턴스를 
		 저장할 수 있는 상자가된다. 
		 */
...
//		ArrayList<Student> list2 = new ArrayList<Student>();
		...
		/*
		 저장할 Student 인스턴스 생성, 외부 패키지에 선언되었으므로 반드시 import후 사용해야한다.
		 */
		...
		list2.add(st3);
		list2.add(st4);//인덱스 3
		//List는 중복저장이 가능하므로 정상적응로 추가된다. 
		list2.add(st2);
		
		/*
		 일반 for문 사용을 위해 size()로 추가된 인스턴스의 갯수를 반환받는다.
		 또한 index로 접근해야 하므로 get()을 사용한다. 
		...
		for(int i = 0; i < list2.size(); i++) {
			/*
			 ...
			 toString()을 오버라이딩 했으므로 인스턴스에 저장된 멤버변수값을
			 출력할 수 잇다. 만약 오버라이딩 하지 않았다면 참조값이 출력될 것이다.
			 */
		...
		/*
		 ...
		 반환할 인스턴스가 있는지 확인한 후 true이면 next()을 통해 출력한다. 
		 */
		...
		for (Student st : list2) {
			/*
			 ...
			 선언되었으므로 getter를 통해 접근한다. 삭제를 위해 입력한 이름과
			 Student인스턴스에 저장된 name과 일치하는지 확인한 후 해당 인스턴스를 찾을 수 있다. 
			 */
