...
/*
 내부클래스(Inner class)
 :클래스 안에 정의된 클래슬 static이 없는 클래스를 말한다.
 -외부 클래스의 모든 멤버(정적 혹은 인스턴스형)를 사용할 수 있다.
 -정적 멤버를 가질 수 없다. 단 상수는 가질 수 있다.
 -컴파일 외부클래스명$내부클래스명.class 파일이 생성된다.
...
class Unit {
	...
	//인스턴형 멤버변수 : 사용시 new를 통해 인스턴스를 생성한다.
	Marine marine;
	Medic medic;
	//생성자
	public Unit(String name, int healthPoint) {
		...
		//인스턴스형 멤버변수 초기화
		marine = new Marine();
		//Medic클래스는 외부에서 인스턴스를 생성해본다(안에서 해도 상관없음).
		//medic = new Medic();
	}
	//멤버변수의 정보를 문자열로 반환한다.
...
public class E01InnerClassMain {
	...
	public static void main(String[] args) {
		...
		/*외부클래스가 아닌 다른 클래스에서는 내부 맴버 클래스가 보이지 않으므로
		  직접 인스턴스를 생성할 수 없다. */
//		Marine m = new Marine();//에러발생
		
		/*외부클래스의 인스턴스를 먼저 생성한 후 내부클래스의 인스턴스를 생성해야한다. 
		  Marine은 Unit클래스의 생성자에서 이미 인스턴스 생성이 되어 있으므로 아래와
		  같이 사용할 수 있다. */
		...
		/*
		 ...
		 new를 통해 생성해야한다. 이때에도 인스턴스를 통해 생성해야하므로 new를 두번 사용해서 
		 인스턴스를 생성한다. 
		 */
