...
/*
 ...
 즉, 하나의 메모리를 할당한 후 이를 프로그램 전체에서 공유해서 사용하자할 때 쓰는 패턴이다.
 
 작성방법
 1. 생성자의 접근지정자를 private으로 선언한다.
 2. 그러면 클래스 외부에서는 생성자에 접근할 수없으므로 new를 통해 인스턴스를 생성을 할 수 없다. 
...
class NoSingleTone {
	...
	/*생성자는 대부분 public으로 선언한다. 
	  만약 privat으로 선언하면 클래스 외부 혹은 다른 패키지에서는 접근할 수 없기 때문이다. */
	public NoSingleTone() {
	}
}
...
public class E03SingleToneDesignPattern {
	...
	public static void main(String[] args) {
		...
		//생성자가 private으로 선언되어 새로운 인스턴스를 생성할 수 없다. 생성자가 "not visible" 즉 보이지 않는다는 에러가 발생한다.
//		SingleTone st1 = new SingleTone();//에러발생

		/*정적메서드이므로 클래스명을 통해 직접 호출할 수 있다. 해당 메서드 호출을 통해 싱글톤 인스턴스의 참조값을 얻어 올 수 있다. */
		...
		System.out.println(String.format("st2의 share:%d", st2.shareVar));//200
		System.out.println(String.format("st3의 share:%d", st3.shareVar));//200
	}
}
