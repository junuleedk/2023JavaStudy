package ex11static;
/*
 static variable, static method(정적 변수, 정적 메서드)
 ...
 접근방법
 -클래스내부
 	: 변수명으로 접근할 수 있다. 일반적인 멤버변수와 동일하게 접근하다.
 -클래스외부
 	1. 인스턴스 변수를 통해 접근한다. (권장사항 아님)
 	2. 인스턴스 생성없이 클래스명으로 직접 접근하다. (권장사항)
...
class Mystatic {
	/*멤버변수 : 인스턴스형 멤버변수는 반드시 클래스의 인스턴스를 생성한 후 참조변수를 통해 접근해야한다.
	   즉, 일반적인 멤버변수는 인스턴스를 생성하는 시점에 메모르이 로드된다. 
	   (멤버변수는 stack영역에, 인스턴스 Heap영역에 생성됨*/
	int instanceVar;
	
	/*정적멤버변수 : 인스턴스를 생성할 필요없이 클래스명으로 바로 접근할 수 있다. JVM(자바가상머신)이 프로그램을 시작할 때
	 *		 	 이미 메모리에(Method영역)에 로드된다.*/
	static int staticVar;
	
	/*인스턴스형 멤버메서드 : 인스턴스형 메서드에서는 모든 멤버를 사용할 수 있따. 인스턴스형 멤버, 정적 멤버 둘 다 접근이 가능하다. */
	void instanceMethod() {
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		...
		//static(정적) 멤버변수 접근방법: 클래스명을 통해 직접 접근한다. 인스턴스를 생성하지 않고 즉시 접근할 수 있다. 
		...
		/*하지만 정적변수는 메모리(Method영역)에 딱 하나만 생성되므로 어떤 인스턴스 
		  변수로 접근하든 동일한 메모리가 된다. 따라서 값을 공유하게 되므로 동일한 결과를 출력하게 된다.*/
		...
		/*
		 ...
		 정적변수에 접근시에는 참조변수를 사용하지말고, 클래스명을 사용해야한다.
		 */
...
//		Mystatic.instanceMethod();//에러발생
		...
		/*
		 결론: 일반적인 인스턴스형 멤버는 인스턴스 참조변수를 통해 접근한다.
		 	  정적(static)멤버는 클리스명을 통해 인스턴스 생성없이 접근한다.
		 */
	}
