...
/*
 ...
 -행동의 조건
 	...
 	걸으면 에너지가 1 감소
 	생각하면 에너지가 2 감소
 	단, 에너지의 최대/최소 범위는 0 ~ 10 으로 한다.
 	무한히 먹을 수 없고, 에너지는 마이너스가 될 수 없기 때문.
 */
public class Human {
	/*
	 * 멤버변수 : 객체의 속성을 표현한다. 나이의 경우 연산이 필요하지 않아면 String으로 설정해도 된다. 에너지의 경우 증가 혹은 감소
	 * 연산이 필요하므로 반드시 int로 설정해야한다.
	 */
	String name;
	int age;
	int energy;
	...
	void walking() { // "걷는다"를 표현
		energy--;
		/*
		  걷는 행위는 에너지를 1 감소시킨다. 만약 에너지를 차감한 결과가 음수라면 해당 시나리오의 조건에 위배되므로 0으로 고정하는 코드가
		  포함되어 있다.
		 */
		if (energy < 0) {
