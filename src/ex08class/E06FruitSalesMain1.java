...
class FruitSeller {
	...
	public int saleApple(int money) {
		...
		this.numOfApple -= num;
//		numOfApple -= num; // 이렇게 사용해도 됌
		this.myMoney += money;
//		myMoney += money; // 이렇게 사용해도 됌
		return num;
	}
	//판매자의 현재상태를 출력한다. 
...
class FruitBuyer {
	...
	/*
	 ...
	 2. 판매자가 금액에 해당하는 사과의 갯수를 반환하면, 해당 갯수만큼 보유한 사과가 증가한다. => += 메서드 호출(5000)
...
public class E06FruitSalesMain1 {
	...
	public static void main(String[] args) {
		...
		/*생성자를 만들지 않았으므로 자동으로 생성되는 디폴트 생성자를 통해 인스턴스를 생성하게 된다.*/
		...
		System.out.println("구매행위가 일어난 후 의 상태");
		//재고수량 : 95개, 수익금 : 5000원
		seller.showSaleResult();
		//구매수량 : 5개, 보유금액 : 0원
		buyer.showBuyResult();
