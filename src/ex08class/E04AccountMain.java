...
class Account {
	//멤버변수
	String name;
	//계좌번호는 연산의 대상이 아니므로 String으로 선언하다.
//	String accountNumber;
	
	/*만약 계좌번호를 정수형태로 표현하고 싶다면 아래와 같이 long타입응로 선언하면 된다.
	  이 경우 가독성을 위해 숫자 사이에 _(언더바)를 사용하는 것이 허용된다. 단, 정수타입이므로
	  작성시 ""은 제거해야한다. */
	long accountNumber;
	/*계좌번호나 전화번호같은 데이터를 표현할 때는 위와같이 String타입이나 long타입을 통해 표현할 수 있다.*/
	//잔고의 경우 입출금을 위한 연산이 필요하므로 int로 선언하다.
	int balance;
...
//		account.accountNumber = "123-45-67890";
		account.accountNumber = 123_45_67890;
		account.balance = 900000;
		/*
		 멤법ㄴ수를 직접 기술하는 것은 초기화메서드에 비해 불편한 방법이므로 
		 첫번째 방법으로 초기화 하는 것이 좋다.
		 */
		account.showAccount();
	}
