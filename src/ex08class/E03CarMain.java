...
class Car {
	...
	/*
	 ...
	 즉 객체를 표현하는 또 하나의 자료형을 생성하는 것이 class를 작성하는 목적이된다.
	 */
	...
	void initialize() {
		...
		carModel = "람보르기니";
		/*
		 자동차 소유주를 표현한 Human타입의 멤버변수는 아직 인스턴스가 생성되지 않은 상태이므로, 
	...
	void drive() {
		/*
		 owner는 인스턴스이므로 이름을 출력하기 위해 '변수명.name'과 같이 작성해야한다.
		 */
	...
	public static void main(String[] args) {
		...
		/*
		 자동차2 객체 생성 : 초기화 메서드를 호출하는 대신 멤버변수에 직접 접근하여 인스턴스를 초기화한다.
		 				이 경우 인스턴스를 생성할 때 마다 동일한 코드를 작성해야 하므로 중복이 많이지게되어 비효율적이다. 
		...
		/*
		 ...
		 이 경우 매개변수를 통해 다양한 값을 전달 할 수 있으므로 여러형태의 인스턴스를 생성할 수 있다. 
		...
		/*
		 해당 인스턴스는 생성만 하고 초기화하지 않은 상태로 출력을 시도하므로 예외가 발생하게된다.
