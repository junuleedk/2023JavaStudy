...
/*
 ...
 -따라서 배열명(생성함수)는 주소값(참조값)을 할당받게 된다.
...
public class E01OneDimArray01 {
	...
	public static void main(String[] args) {
		...
		// 정수형 배열이 Heap 영역에 생성된 후 반환된 주소값이 출력된다.
		System.out.println("배열명(numArr): " + numArr);
		System.out.println("=============================");
		/*
		 * 배열의 선언 및 초기화 : 배열은 여러개의 변수를 사용하기위해 선언하므로 접근을 위해 일반적으로 반복문(for)을 사용한다.
		 * 
		...
		System.out.println("=============================");
		/*
		 * 인스턴스배열: 기본자료형을 저장하는 배열이 아닌 인스턴스의 참조값을 저장할 목적으로 생성한 배열을 말한다. 사용법은 일반배열과 동일하다.
		 */
		...
		/*
		 * 배열을 선언과 동시에 초기회하는 경우 자바 컴파일러는 초기값의 갯수를 통해 배열의 크기를 결정할 수 있다. 따라서 이 경우에는 크기는
		 * 생략하도록 약속되어있다.
		 */
		System.out.println("방법2: 배열의 선언 이후 초기화1");
//		int[] initArr2 = new int[3] {1, 20, 300}; //에러발생
		int[] initArray2 = new int[] { 1, 20, 300 };

		/*
		 * 배열 원소의 갯수를 통해 배열을 생성한 후 초기화까지 동시에 진행한다. 이 경우 new를 생갹할 수 있다.
		 */
