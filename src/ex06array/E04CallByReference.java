...
/*
 ...
 메모리(Heap영역)을 참조할 수 있게 하는 호출방식이다. 
 메서드의 지역이 다르더라도 참조값을 통해 인스턴스를 참조하게되므로 
 A지역에서의 값의 변경이 B지역에서도 그대로 참조되는 장점이 있다.
 참조값만 전달하므로 프로그램의 퍼포먼스 (성능)도 향상되고, 메모도 절약할 수 있다.
 */
public class E04CallByReference {
	...
	public static void main(String[] args) {
		...
		int[]arr = { 100, 200 };
		System.out.println("[메인메서드]- Swap 전 출력");
		//출력1: 100, 200
		...
		System.out.println("[메인메서드]-Swap 이후 출력");
	...
	static void callByReference(int[]ref) {
		/*
		 매개변수를 통해 전달받은 배열인스턴스의 참조값을 통해 0번 원소와 1번 원소를 서로 교환한다. 
		...
		System.out.println("[callByReference]-Swap 이후 출력");
