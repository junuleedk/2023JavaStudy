...
class PersonalNumberStorageExt extends PersonalNumberStorageAbs {
	...
	public PersonalNumberStorageExt(int arrSize) {
		...
		numOfPerInfo = 0;
	/*
	 상속을 통해 추상메서드를 포함하게 되므로 반드시 아래와 같이 오버라이딩해야한다.
	 이는 필수사항이며 하지 않을 경우 에러가 발생하게 된다. 	
	 */
	}
	@Override
	...
	public String searchPersonalInfo(String juminNum) {
		...
		for(int i = 0; i < numOfPerInfo; i++) {
			/*주민번호가 일치하는지 판단한다. 주민번호는 private으로 선언된 멤버이므로 
			직접 접근이 불가능하다. 따라서 아래와 같이 getter를 통해 값을 반환받아 사용해야한다.*/
...
public class E02AbstractToInterface1 {
	...
	public static void main(String[] args) {
		...
		storage.addPersonalInfo("901234-122222", "정지훈");
		
		//검색 
		System.out.println(storage.searchPersonalInfo("901234-222222"));
		System.out.println(storage.searchPersonalInfo("901234-122222"));
