...
/*
 ...
 : 메인쓰레드와 Working쓰레드(개발자가 정의한 쓰레드)를 일컫는 것으로
   메인쓰레드가 종료되어도 계속 실행되며, Dead상태가 되어야만 실행이 종료되는 쓰레드를 말한다.
   해당 쓰레드는 main() 메서드가 종료된 이후에도 계속 실행되는 것을 볼 수 있다. 
 */
class NormalThread extends Thread {
	/*기본 생성자를 통해 인스턴스를 생성하면 쓰레드명이 JVM에 의해 자동으로 부여된다. */
	public NormalThread() {}
	...
	/*
	 쓰레드 동작 중 비정상적으로 종료될 때 해당 예외가 발생할 수 있으므로
	 예외처리를 하거나 아래와 같이 예외던지기를 해서 호출한 위치로 예외 객체를 
	 던질 수도 있다. 
	 */
...
/*
 ...
 : 모든 독립쓰레드가 종료되면 자동으로 종료되는 쓰레드를 말한다. 
   ...
   주로 프로그램의 보조역할로 배경음악재생, 로그자동저장 등의 업무
   처리한다. 프로그램이 종료될 때 함께 종료되므로 무한루프로 구현한다. 
 */
class DamonThread extends Thread {
	/*
	 무한루프로 구성되어 있고 별도의 탈출 조건을 만들지 않는다. 3초에 한번씩 블럭상태로 들어간다.
	 */
	@Override
	public void run() {
		while(true) { 
...
public class Ex03DaemonThread {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 순차적으로 실행되어 1번 인스턴ㅌ스가 작업을 마쳐야만 2번째가 실행된다. 
		 */
		...
		/*
		 데몬쓰레드를 독립쓰레드를 먼저 생성한 후 setDamon()을 통해
		 만들 수 있다. 
		 */
		...
		/*
		 Thread 클래스의 정적메서드를 통해 쓰레드의 여러상태를 확인 할 수 있다.        
		 */
