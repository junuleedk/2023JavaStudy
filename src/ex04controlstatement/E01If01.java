...
// 		}
 		...
 		// 2가지 조건 모두 만족하므,로 print()문이 실행된다.
 		if(num%2 == 0 && num >= 10) {
 		...
 		if(true) {
 			System.out.println("무조건 실행되는 if문이다.");
 		}
 		...
 		/*
 		 조건 검사 후 실행할 문장이 1개인 경우에는 중고라호를 생략할 수 있다.
 		...
 		/* 아스키코드를 알고 있을 때의 구현방법, 문자 0과 9가 아스키코드 48, 57에 해당하므로
 		   구간 내에 있는지 확인하면 된다.
 		 */
 		...
 		/* 아스키코드를 모를 때 구현하는 방법으로 문자 '0'과 '9'는 메모리에 저장시 아스키코드로 변화되므로 
 		 아래와 같은 조건으로도 확인할 수 있다.
 		 */
