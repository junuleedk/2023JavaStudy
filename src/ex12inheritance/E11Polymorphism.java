...
/*
 다형성(Polymorphism)
 : 하나의 인스턴스를 여러 타입의 참조변수를 통해 참조할 때
 ...
 4. 이것을 이질화(Heterogenous: 헤테로지니어스) 라고 한다.
...
class Parent {
	...
	void parentMethod() {
		System.out.println("부모메서드:parentMethod()");
	}
}
...
class Child extends Parent {
	...
	/*
	 * 메서드명은 동일하지만 매개변수가 틀리므로 오버로딩으로 정의된 메서드 따라서 자식에서 확장한 멤버가 된다.
	 */
	...
	public static void main(String[] args) {
		...
		/*
		 이질화: 부모타입의 변수로 자식인스턴스를 참조하는 것을 말한다.
		       이 경우 자식클리스에서 새롭게 정의한 멤버는 접근할 수 없다.
...
//		parent1.childMember = 1;
		...
		/*부모 참조변수로 접근하려면 강제형변환 해야한다. 
		  ...
		  소괄호를 하나 더 추가해서 parent2를 자식타입으로 
		  형변환 후 멤버에 접근해야한다.*/
		...
		System.out.println("[클래스의 끝판왕 Oblject]");
		/*Java에서 생성한 모든 클래스는 Object클래스를 상속한다. 따라서 모든 인스턴스는 Object의 참조변수로 참조가 가능하다. 또한 
		  Object클레스에 정의된 모든 메서드를 별도의 선언없이 사용할 수 있고 필요하다면 오버라이딩도 할 수 있다. */
		Object object = new Child();//crtl + t
