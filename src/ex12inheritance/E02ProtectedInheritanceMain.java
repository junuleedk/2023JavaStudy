...
/*부모클래스와 다른 패키지에 선언되었으므로 import 해야한다.
  단, 부모클래스는 반드시 public으로 선언해야한다. 만약 default로 선언하면 접근자체가 불가능하므로
  import도 할 수 없게된다. */
...
class Seoul extends Korea {
	...
	public Seoul(String name, String city) {
		...
		super(name);//부모생성자 호출( 위 아래 위치 바꾸면 안됨)
		this.city = city;
	}
	/*4개의 접근지정자를 통해 정의 한 메서드를 호출한다. 단 부모 클래스가 다른 패키지에 선언되었으므로
...
//		super.privateMethod();//발생
		/*이와같이 직접 접근할 수 없는 메서드는 접근 가능한 멤버메서드를 통해 간접호출하면된다. */
	}
}
