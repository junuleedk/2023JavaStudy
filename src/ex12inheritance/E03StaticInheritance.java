...
/*
 ...
 단, 클래스외부에서 접근할 경우 인스턴스 생성없이 클래스명을 통해서 접근할 수 있다. 
 즉, static이 가진 기본규칙을 따른다. 
 */
class Adder {
	/*static으로 선언된 멤버변수는 실제로는 클래스외부(Method영역)에 생성되어
	...
	public void add(int num) {
		val += num;
		Adder.val += num;//명시적인 코드가 더 좋은 코드라 이방법을 추천한다.
	}
}
//부모의 멤버변수가 public이므로 접근의 제한이 없다. 따라서 자식 클래스에 작성된 모든 코드는 문제없이 실행된다. 
...
public class E03StaticInheritance {
	...
	public static void main(String[] args) {
		...
		/*
		 앞에서 호출한 메서드는 모두 메서드영역에 
		 생성된 정적변수 val에 더해지게 되므로 출력결과는 20이된다. 
		 */
		adFriend.showVal();
	}
