...
public class E07RideAndLoad02 {
	...
	public static void main(String[] args) {
		...
		/*오버라이딩 처피된 메서드이므로 참조변수와 상관없이 항상 자식쪽 메서드가 호출된다. 
		...
		/*
		 ...
		 자식인스턴스가 생성될 때 부모쪽의 메서드가 가려지게 되므로 참조변수의 영향을 전혀받지 않는다. 
		 */
		parent.exercise();
		parent.sleep();
		parent.printParent();
		
		/*부모클래스의 참조변수로 자식인스턴스를 참조하면 부모의 영역까짐잔 접근할 수 있으므로
		  자식영역에 정의된 메서드는 호출할 수 없다. */
//		parent.study();
		parent.walk();
//		parent.walk(20);
		
		//정적메서든ㄴ 오버라이딩 되지 않는다. 클래스명으로 호출하면된다.
