...
class Speaker {
	...
	public void showState() {
		System.out.println("스피커의 불륨크기: " + volumnRate);
	}
}
...
class BaseSpeaker extends Speaker {
	...
	public void showState() {
		...
		super.showState();//super를 꼭 붙여서 자기 자신이 아니라 부모쪽의 메서드임을 말한다.
...
public class E06Overriding {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 윈도우 탐색이에서 같은 이름의 파일을 붙여넣기 할 때 덮어쓸지 물어보는 것과 동일하다 
		 생각하면 된다.
		 */
		...
		/*부모클래스의 참조변수로 자식인스턴스를 참조할 수 있다. 
		  이것은 상속관계이기 때문에 가능하다. */
		Speaker speaker = new BaseSpeaker();//부모타입인 Speaker로 참조할 때
		speaker.setVolumn(30);
		/*단, 부모타입의 변수로 자식 인스턴스를 참조하면 접근할 수 있는 범위가 부모인스턴스까지로 제한되므로,
		  자식쪽에 정의된 setBase()는 호출할 수 없다. */
//		speaker.setBase(40);//에러발생 - 부모로 인스턴스 접근하기 때문에 접근범위가 자식까지는 가지 못함.
		/*접근범위는 앞에서 설명한 바와 같이 브모까지로 제한되지만 오버라이딩 된 메서드가 있다면 자식쪽의 메서드가 호출된다.*/
		speaker.showState();//자식showState() 메서드 호출
	}
