...
class Computer {
	...
	/*개발자가 생성자를 정의하면 다폴트 생서자는 자동으로 추가되지 않는다
	  즉, 생성자는 아래와 같이 필요한 상황에 따라 정의하면 된다. */
	//public computer() {} // -> new Computer();
	public Computer(String name) { // -> new Computer("낙자");
		owner = name;
	}
	/*private 접근지정자로 선언된 멤버메서드는 외부클래스 혹은 상속받은
	  클래스에서 접근이 가능하므로 , public으로 선언된 메서드를 통해 간접적으로 호출해야한다. */
	...
	/*private으로 선언된 멤버메서드를 외부에서 호출할 수 있게 public으로 지정하여 선언한 메서드로 
	  "캡술화" 되어있다. */
...
class NotebookComputer extends Computer {
	...
	public NotebookComputer(String name, int initCharge) {
		/*부모클래스의 생성자를 호출하기 위해 사용하는 super()는 생략할 수 있지만, 앞에
		  다른 문장이 먼저 기술되면 에러가 발생한다. 또한 여기서는 부모클래스에 디폴트 생성자가 없는
		  상태이므로 생략하는 즉시 에러가 발생한다.*/
//		super();//에러발생
		super(name);
		//멤버변수 초기화
		battary = initCharge;
	...
	public void movingCal() {
		...
		if(battary < 1) {
			...
			/*메서드에서 아래와 같은 return을 만나게되면 실행이 중지(종료)된다.*/
			return;
		}
...
class TabletNotebook extends NotebookComputer {
	...
	public void write(String penInfo) {
		 ...
		 * A.compareTo(B)
		   : 문자열 A와 B를 비교하여 0이 반환되면 동일한 문자열, -1 혹은 1이 반환되면 서로 다른 문자열로
		     판단하는 String 클래스에 정의된 메서드 */
		if(registPencil.compareTo(penInfo) != 0) {
			System.out.println("등록된 펜이 아닙니다.");
			return;
		}
		/*부모클래스에 정의된 메서드를 호출한다. 상속받은 자식클래스에서는 이와같이
		  부모의 멤버메서드를 호출하여 자신의 멤버처럼 사용할 수 있다. 
		  단 접근이 가능한 접근지정자로 선언되어야 한다. super는 생략할 수 있다. */
		super.movingCal();
