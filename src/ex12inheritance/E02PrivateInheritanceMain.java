...
class Account {
	/*
	 멤버변수가 상속관계에 있다하덜다도 private으로 선언되면 클래스 내부에서만 접근할 수 있다.
	 즉, 상속받은 하위클래스에서는 직접 접근이 불가능하다. 
	 */
	private int money;
	/*이와같이 개발자가 생성자를 직접 정의하면 디폴트 생성자는 추가되지 않는다. 디폴트 생성자는 
	  개발자가 생성자를 정의하지 않았을때만 컴파일러에 의해 자동으로 추가된다. */
	...
	/*현재는 부모와 자식 클래스가 동일한 패키지에 정의되어 있으므로 접근지정자가 default이더라도 접근 허용된다. 
	  즉, protected를 여기에선 지워도 상관없다.*/
...
class SavingAccount extends Account {
	...
	public SavingAccount(int initVal) {
		/*만약 아래 문장을 삭제하면 즉시 에러가 발생한다. 삭제하는 경우 super()라는 문장, 
		  ...
		  부모클래스에느는 이런 형태의 생성자가 없으므로 아래와같이 명시해야한다. */
		super(initVal);
//		super(;)
	}
	/*
	 부모클래스에 정의된 멤버변수 money는 private이므로 자식쪽에서는 아래와 같이 직접 접근할 수 없다. 보이지 않으므로 에러가 발생한다. 
...
//		sa.money = 100000;//에러발생
		...
		/*부모클래스를 통해 인스턴스를 생성한다. 하지만 해당 인스턴스 변수를 통해서도 멤버변수 money에 접근할 수 없다. 현재 접근하려는 위치가 클래스 외부이기 때문이다.
