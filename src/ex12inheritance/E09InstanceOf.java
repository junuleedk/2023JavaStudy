...
/*
 instanceOf 연산자
 : 인스턴스 변수가 어떤 타입의 변수인지를 판단하는 연산자로 형변환 (즉 상속관계가 있으면)
 이 가능하면 true를 반환한다.
 -인스턴스간에 형변환이 되려면 반드시 두 클래스간에 상속관계가 있어야한다.
 -부모타입의 참조변수로 자식 인스턴스를 참조할 수 있으므로 매개변수를 부모타입으로 만들면
 자식 인스턴스를 인수로 받을 수 있다. 
 -매개변수로 전달되는 인수는 부모타입으로 자동형변환(업캐스팅)되므로 정상적인 사용을 위해 강제형변환(다운캐스팅)
 해야하는ㄴ데 이때 확인을 위해 사용된다. 
 */
...
public class E09InstanceOf {
	...
	/*
	 ...
	 -box타입으로 인수를 받게되면 모두 자동형변환(업캐스팅) 되므로 자식 클래스의 
	 멤버메서드는 호출할 수 없게된다. 
	...
	static void wrapBox(Box b) {
		...
		/*
		 세가지 인스턴스 중 어떤 것이 전달되더라도 Box타입으로 자동형변환되므로 
...
//		b.boxWrap();
	...
	/*
	 ...
	 복잡해지게 될 것이다. 따라서 부모로 자식을 참조할 수 있는 것은 개발자에게
	 편리함을 가져다준다. 밑에 내용은 위의 내용과 같은데 갯수가 많아지면 불편해진다.
	 */
