...
/*
 캡슐화(Encapsulation)
 : 여러가지 업무로직을 하나로 묶는다는 의미와, 업무의 순서까지 고려한 형태의 로직을 
...
/*
 ...
 차우에는 별도의 분석 작업없이 해당 클래스의 doProcess()메서드만 호출하면 
 업무는 차질없이 진행될 것이다.
 */
class EncapsulLogic {
	...
	//업무의 순서를 고러하여 캡슐화된 메서드를 정의한다.
	void doProcess() {
		memberRegist.doMemberRegist();//가입
		autoLogin.doAutoLogin();//로그인
		congratulationPoint.doCongratulationPoint();//포인트지급
...
public class E05Encapsulation {
	...
	public static void main(String[] args) {
        ...
        /*
         업무의 진행순서가 중요하다고 가정했을때 캡술화 이전의 코드에서는 개발자가 순서를 지키지 않은 상태에서도 실행이 가능한 코드가 ㅁ된다.
         즉, 업무의 차질이 발생될 수 있다.
         */
		MemberRegist memberRegist = new MemberRegist();
		FirstLoginEvent firstLoginEvent = new FirstLoginEvent();
