...
public class E06LocalVariable {
	...
	public static void main(String[] args) {
		...
		boolean scope = true;
		/*
		 * main지역에서 num을 선언하면 그보다 좁은 지역인 if문에서는 같은 이름의 변수를 선언할 수 없다. 큰 지역에서 이미 선언되어 메모리에
		 * 상주하고 있기 때문이다.
		 */
//		int num = 9;

		/*
		 * main보다 좁은 지역인 if문의 블럭을 생성한 후 변수 num을 선언한다. 단, if문과 else문은 서로 다른 지역이므로 동일한 이름의
		 * 변수를 선언할 수 있다.
		 */
		if (scope) {
			int num = 1;
			...
			int num = 5;
			System.out.println("첫번째 if문의 else 지역ㅣ " + num);
		}
		/*
	...
	/*
	 * main에서 선언한 변수 num은 메모리에 아직 상주하고 있지만 해당 메서드는 크기가 동일한 서로 다른 지역에 정의되었으므로 같은 이름의
	 * 변수를 선언할 수 있다. 앞의 main메서드의 if ~ else문의 블럭도 같은 개념으로 변수를 선언한 것이다.
	 */
